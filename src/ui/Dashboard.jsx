...
const Dashboard = ({ isLoading, error, booksData }) => {
  const [booksReviewed, setBooksReviewed] = useLocalStorage([], 'booksReviewed');

  const [bookSelected, setBookSelected] = useState({});

  const [isActiveTab, setIsActiveTab] = useState(booksReviewed?.length ? 1 : 0);

  //switch tabs
  const handleTab = (tabSelected) => {
    setIsActiveTab((isActiveTab) => (isActiveTab === tabSelected ? isActiveTab : tabSelected));
  };
  ...
  const handleReviewedList = (book) => {
    ...
    if (!booksReviewed?.length > 0 || !bookFound) {
      setBooksReviewed((booksReviewed) => (book.rated ? [...booksReviewed, book] : booksReviewed));
    }
  };
  ...
  const onDeleteBook = (book) => {
    ...
    setBooksReviewed((booksReviewed) =>
      booksReviewed.filter((bookReviewed) => bookReviewed.key !== book.key),
    );
  };
          ...
          {Object.keys(bookSelected)?.length > 0 || booksReviewed?.length > 0 ? (
            ...
            >
              {Object.keys(bookSelected).length > 0 && (
                <Button nameClass={isActiveTab === 0} handleClick={() => setIsActiveTab(0)}>
                  Book view
                </Button>
              )}
              <Button nameClass={isActiveTab === 1} handleClick={() => setIsActiveTab(1)}>
                Books Reviewed
              </Button>
            </ReviewContainer>
          ) : null}
